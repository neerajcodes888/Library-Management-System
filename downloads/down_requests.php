...
{
    $conn = connectDB();
    $stmt = $conn->prepare("select request_no,book_no,book_name,book_author,status from request_books where student_id='$_SESSION[id]'");
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return $result;
}
...
$pdf->Ln(10);
$pdf->Cell(90, 10, 'Requested Books List', 1, 1, 'C');
$pdf->Ln(3);
$pdf->AliasNbPages();

$pdf->SetFont('Arial', '', 12);

$bookList = getRequestData();
$pdf->Cell(30, 10, 'Request No.', 1); 
$pdf->Cell(20, 10, 'Book No.', 1); 
$pdf->Cell(40, 10, 'Book Name', 1);
$pdf->Cell(60, 10, 'Author Name', 1); 
$pdf->Cell(20, 10, 'Status', 1); 
$pdf->Ln();

foreach ($bookList as $book) {

    $pdf->Cell(30, 10, $book['request_no'], 1);
    $pdf->Cell(20, 10, $book['book_no'], 1);
    $pdf->Cell(40, 10, $book['book_name'], 1);
    $pdf->Cell(60, 10, $book['book_author'], 1);
